...
define(['backbone', 'common-objects/collections/users'],
    function (Backbone, Users) {
        ...
        var statusHtml = {
            USER_STATUS_OFFLINE: '<i class="fa fa-user user-offline"></i> ' + App.config.i18n.OFFLINE,
            USER_STATUS_ONLINE: '<i class="fa fa-user user-online"></i> ' + App.config.i18n.ONLINE
        };
                    ...
                    reset: true, success: function () {
                        ...
                        if (user) {
                            ...
                            $tip.find('.webRTC_invite_button').one('click', function () {
                                Backbone.Events.trigger('NewOutgoingCall', {
                                    remoteUser: user.get('login'),
                                    context: App.config.workspaceId + ' : ' + context
                                });
                                $(that).popover('hide');
                            });
                            ...
                            $tip.find('.new_chat_session_button').one('click', function () {
                                Backbone.Events.trigger('NewChatSession', {
                                    remoteUser: user.get('login'),
                                    context: App.config.workspaceId + ' : ' + context
                                });
                                $(that).popover('hide');
