...
define([
    ...
    'moment'
], function (Backbone, Mustache, template, PartGroupedByListItemView, moment) {
    'use strict';
        ...
        groupBy: function () {
            ...
            return !this.groupedByList.length ? {'undefined': this.items} : _.groupBy(this.items, function (item) {
                ...
                _.each(self.groupedByList, function (groupByColumn) {
                    var key = item[groupByColumn];
                    try {
                        var dateParsing = moment(key, 'YYYY-MM-DDTHH:mm:ssZ').format(App.config.i18n._DATE_FORMAT);
                        if (dateParsing !== 'Invalid date') {
                            key = dateParsing;
                        }
                    } catch (e) {
                    }
                    groupByStringToUse = groupByStringToUse + ' > ' + key;
                });
